...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Si existe el parámetro 'category', busco el filtro correspondiente y marco 'aria-pressed="true"'
  if (category) {
    const selectedFilter = document.querySelector(`.main__chip[data-category="${category}"]`);
    if (selectedFilter) {
      selectedFilter.setAttribute("aria-pressed", "true");  // Establecer 'aria-pressed' en 'true'
    }
  }
});
...
function createCardPost() {
  ...
  if(category) {
    postsToDisplay = posts.filter(post => post.category.trim().toLowerCase() === category.trim().toLowerCase())
  }
